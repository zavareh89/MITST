...
SELECT
  ...
  , COALESCE(admissionweight, dischargeweight) as weight
  , CASE
    -- CHECK weight vs. height are swapped
    WHEN  COALESCE(admissionweight, dischargeweight) >= 100
      AND COALESCE(admissionweight, dischargeweight) >  25 AND COALESCE(admissionweight, dischargeweight) <= 100
      AND abs(COALESCE(admissionweight, dischargeweight)-COALESCE(admissionweight, dischargeweight)) >= 20
    THEN 'swap'
    END AS method
  FROM patient
)
...
(
  SELECT
    patientunitstayid
    , chartoffset
    , 'admit_discharge' as weight_type
    , CASE
...
(
  ...
  and nursingchartvalue <> '' -- Filter out empty strings
